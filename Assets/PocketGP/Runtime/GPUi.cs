...
public sealed class GPUi : MonoBehaviour {
    ...
    RectTransform Card(string eyebrow, string title) {
        ...
        Txt(card, eyebrow, new Vector2(0, 350), new Vector2(620, 30), 18, mint);
        Txt(card, title, new Vector2(0, 295), new Vector2(640, 85), 43, ink);
        return card;
    }
    ...
    public void ShowMenu() {
        ...
        Txt(c, best > 0 ? "Record : " + TimeText(best) : "3 tours  ·  5 pilotes  ·  armes bonus", new Vector2(0, 80), new Vector2(620, 30), 18, muted);
        string[] dif = { "Facile", "Moyen", "Difficile" };
        for (int i = 0; i < 3; i++) {
            int k = i;
            Button(c, dif[i], new Vector2((i - 1) * 204, 18), new Vector2(194, 50), () => { game.Difficulty = k; ShowMenu(); }, game.Difficulty == i);
        }
        string[] names = { "Corail", "Menthe", "Citron", "Lilas", "Rose" };
        Button(c, "Voiture : " + names[game.ColorIndex] + "   ›", new Vector2(0, -47), new Vector2(604, 50), () => { game.ColorIndex = (game.ColorIndex + 1) % 5; game.BuildWorld(game.TrackIndex); ShowMenu(); });
        ...
        Button(c, "Comment jouer", new Vector2(-154, -269), new Vector2(296, 52), ShowHelp);
        Button(c, "Audio & affichage", new Vector2(154, -269), new Vector2(296, 52), () => ShowSettings(false));
    ...
    public void ShowSettings(bool paused) {
        ...
        Button(c, "Qualité : " + (QualitySettings.shadows == ShadowQuality.Disable ? "Éco" : "Élevée"), new Vector2(0, -143), new Vector2(604, 56), () => {
            ...
            QualitySettings.antiAliasing = eco ? 0 : 2;
            ShowSettings(paused);
        });
    ...
    public void ShowResults() {
        ...
        Txt(c, game.ResultPlace + " / 5", new Vector2(0, 155), new Vector2(600, 110), 76, mint);
        Txt(c, "Temps : " + TimeText(game.FinalTime) + "\nJetons : " + game.Score + " points", new Vector2(0, 42), new Vector2(600, 90), 28, ink);
        ...
        if (game.Championship) Button(c, game.TrackIndex < 2 ? "CIRCUIT SUIVANT" : "RÉSULTAT DU CHAMPIONNAT", new Vector2(0, -136), new Vector2(604, 64), () => game.Next(), true);
        else Button(c, "REJOUER", new Vector2(0, -136), new Vector2(604, 64), () => game.Retry(), true);
